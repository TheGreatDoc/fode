...
uint GetKarmaTraitBonus(CritterMutual& cr);
...
EXPORT int getParam_Strength(CritterMutual& cr, uint)
{
	...
	if(cr.Params[TRAIT_MAN_OF_HONOR]) val += GetKarmaTraitBonus(cr);
	if(cr.Params[PE_ADRENALINE_RUSH] && getParam_Timeout(cr, TO_BATTLE) && // Adrenaline rush perk
		cr.Params[ST_CURRENT_HP] <= (/*40 + */cr.Params[ST_MAX_LIFE] + cr.Params[ST_STRENGTH] + cr.Params[ST_ENDURANCE] * 2) / 2) val++;
...
EXPORT int getParam_MaxLife(CritterMutual& cr, uint)
{
	...
	//if(cr.Params[ST_IS_PLAYER]==1) val+=40;
	if(cr.Params[ST_IS_PLAYER]==1 && val>480) val=480;
...
EXPORT int getParam_MaxWeight(CritterMutual& cr, uint)
{
	...
	if(cr.Params[SK_RESOURCING]>50) val += int(cr.Params[SK_RESOURCING]/10)*1000;
...
EXPORT int getParam_Ac(CritterMutual& cr, uint)
{
	...
	if(cr.Params[PE_HTH_EVADE]!=0) val+=(getParam_Ap(cr,0)*3+(cr.Params[SK_UNARMED]/12));
...
EXPORT void changedParam_Reputation(CritterMutual& cr, uint index, int oldValue)
{
	if(oldValue == 0x80000000) const_cast< int* >( cr.Params )[ index ] += 0x80000000;
}

EXPORT int getParam_CritType(CritterMutual& cr, uint index)
{
	return cr.Params[ST_IS_PLAYER]!=0?(cr.Params[PLAYER_MAIN_FACTION]==1?(cr.Params[ST_GENDER]==GENDER_MALE?144:145):(cr.Params[ST_GENDER]==GENDER_MALE?69:180)):cr.Params[ST_BASE_CRTYPE];
}
...
int GetNightPersonBonus()
{
	if(FOnline->Hour < 6 || FOnline->Hour > 18) return 2;
	if(FOnline->Hour == 6 && FOnline->Minute == 0) return 2;
	if(FOnline->Hour == 18 && FOnline->Minute > 0) return 2;
	return -1;
}

uint GetKarmaTraitBonus(CritterMutual& cr)
{
	if(cr.Params[ST_KARMA]>250) return 1;
	else if(cr.Params[ST_KARMA]<-250) return -1;
	return 0;
}
